...
sub gen_header {
    my $sym;
    my $syms = shift;
    print "void __attribute__((constructor)) heim_load_functions(void);\n";
    foreach $sym (@$syms) {
...
sub gen_loader {
    ...
    print "\n";
    print "static void load(void) {\n";
    print "void *hf = dlopen(\"/usr/heimdal/lib/libkrb5.so\", RTLD_LAZY|RTLD_LOCAL);\n";
    print "void *gf = dlopen(\"/usr/heimdal/lib/libgssapi.so\", RTLD_LAZY|RTLD_LOCAL);\n";
    foreach $sym (@$syms) {
	my $lib;
	if ($sym =~ /gss/) {
	    $lib = "gf";
